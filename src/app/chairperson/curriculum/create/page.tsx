...
export default function CreateCurriculum() {
  ...
  const [concentration, setConcentration] = useState('Software Development');
  const [isUploading, setIsUploading] = useState(false);
  const [uploadedFile, setUploadedFile] = useState<File | null>(null);  const parseCSVLine = (line: string): string[] => {
    const result: string[] = [];
    let current = '';
    let inQuotes = false;
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      sessionStorage.setItem('uploadedCourses', JSON.stringify(courses));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!curriculumName || !totalCredits) {
      alert('Please fill in curriculum name and total credits');
      return;
    }
    if (!uploadedFile) {
    ...
    // File upload will handle navigation
  };
        ...
        <div className="w-full max-w-5xl mx-auto bg-card rounded-2xl border border-border p-12">
          <div className="flex gap-12">
            ...
            <div className="flex-1 flex flex-col justify-start">              <div className="bg-card p-6 rounded-xl border border-border">
                ...
                <div
                  className={`h-52 border-2 border-dashed rounded-lg flex flex-col items-center justify-center cursor-pointer transition ${
                    uploadedFile 
                  ...
                  {isUploading ? (
                    ...
                    <>
                      <svg className="w-10 h-10 mb-2 text-muted-foreground" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                        ...
                        Drag and drop Excel or CSV file here,<br />
